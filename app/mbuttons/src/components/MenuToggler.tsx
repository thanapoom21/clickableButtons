...
function MenuToggler () {
    const menuToggleRef = useRef<any>(null)
    const openClasses = useMemo<any>(() => {
            return ['animate__slideInLeft']
        }, []),
        closeClasses = useMemo<any>(() => {
            return ['animate__slideOutLeft', '-left-full']
        }, [])

    const changeSidebarClassesCallback = useCallback((add: any, remove: any) => {
    ...
    const clickOutsideSidebarCallback = useCallback((e: any) => {
        const menu = document.getElementById('sidebar')
    ...
    const clickSidebarLinks = useCallback((e: any) => {
